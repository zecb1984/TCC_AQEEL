<?php
	...
	{	//VERIFICA SE FOI DIGITADO UM PARÂMETRO DE PESQUISA. SE NÃO, LISTA TODOS OS CLIENTES
		$sql_consulta = "SELECT count(*) as quantos FROM instrutor ";
		
		if(trim($_POST["instrutor"]) != "") 
		{ //CONSULTA O CLIENTE PELO O NOME OU PELO CPF
		$sql_consulta .= "	where cpfInstrutor like '%".$_POST['instrutor']."%' 
				or lower(nomeInstrutor) like '%".mb_strtolower($_POST["instrutor"],'UTF-8')."%' ";
		}		
		...
		or lower(nomeInstrutor) like '%".$_POST['instrutor']."%' ";
					  
		  $result_lista = mysqli_query($conn, $sql_lista);
		
		  while($row = mysqli_fetch_assoc($result_lista)) 
		  {//MONTA A LISTA PARA EXIBIR NA TELA
			$cpfs[] 	   = $row["cpfInstrutor"];
			$nomes[] 	   = $row["nomeInstrutor"];			
			$rgs[] 		   = $row["identidadeInstrutor"];
...
?>		
    <tbody>
      <tr>
        <th scope="row">
          <input type="radio" name="selecao" value="<?php echo $cpfs[$i];?>" id="selecao" onClick="preencheBotoesConsultaInstrutor(this.value);" />
        </th>
        <td><?php echo $nomes[$i];?></td>
        <td><?php echo $cpfs[$i];?></td>
        <td><?php echo $rgs[$i];?></td>
        <td><?php echo $atividades[$i];?></td>		
      </tr>
    </tbody>
<?php
	}
...
?>	
  ...
  <a class="btn btn-primary btn-lg" href="instrutor.php?acao=cadastro" role="button">Cadastrar</a>
<?php 
if($quantos > 0) 
{
?>
